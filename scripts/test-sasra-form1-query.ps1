...
try{
 ...
 $source=Get-Content '../SwiftFinancialz2/Application.MainBoundedContext/AccountsModule/Services/SasraSetupAppService.Form1.cs' -Raw
 $sql=($source -split 'public const string Form1BalancesSql=@"',2)[1].Split(@('";'),[StringSplitOptions]::None)[0]
...
'@
 ...
 if($table.Rows.Count -ne 3){throw 'Expected all three accounts.'}
 $income=@($table.Rows|Where-Object AccountType -eq 4000)[0];$expense=@($table.Rows|Where-Object AccountType -eq 5000)[0];$cash=@($table.Rows|Where-Object AccountType -eq 1000)[0]
 if($income.Balance -ne -994 -or $income.BeforeYear -ne -999 -or $income.CurrentClosingBalance -ne 175 -or $expense.Balance -ne 0 -or $expense.CurrentClosingBalance -ne -40 -or $cash.Balance -ne 130){throw 'Form 1 opening/date/closing calculation mismatch.'}
 Write-Output 'PASS: actual Form 1 SQL includes cumulative balances, separates prior-year balances and closing transfers, includes the full last day, honors value date, falls back to created date and nets ordinary reversals. No database writes.'
