...
assert.equal(reportRows('DT').length,6);
...
assert.deepEqual(availableReportRows([]).map(r=>r.reportCode),['FORM 1','FORM 2','FORM 3','FORM 4','FORM 6','FORM 7']);
...
console.log('PASS: empty database still lists six implemented reports; saved workbook mappings override placeholders without catalogue duplicates.');
