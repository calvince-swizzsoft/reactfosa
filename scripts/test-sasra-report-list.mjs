...
assert.equal(reportRows('DT').length,8);
...
assert.deepEqual(availableReportRows([]).map(r=>r.reportCode),['FORM 1','FORM 2','FORM 3','FORM 4','FORM 5','FORM 6','FORM 7','FORM 9']);
...
console.log('PASS: empty database still lists eight implemented reports; saved workbook mappings override placeholders without catalogue duplicates.');

assert.equal(availableReportRows([], 'DT').length, 8);
...
assert.equal(availableReportRows([], "DT").find(r=>r.reportCode==="FORM 9").endpoint,"form9");