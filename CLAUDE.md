...
| Accounts (most list pages) | **Operational**, but self-wrap step is often skipped — see below | `Accounts/CostCenters/index.jsx`, `Accounts/ChartOfAccounts/index.jsx` |
...
- Some Accounts list pages (`CostCenters`, `ChartOfAccounts`) skip the self-wrap card entirely — bare `<div>`, no header bar, page content sits directly on `Layout`'s own background. Both are in active use; prefer the self-wrapped card (more consistent, matches the majority) unless matching an existing bare-div sibling in the same folder.
