...
import { FaEdit, FaPlus, FaChevronLeft, FaChevronRight, FaTruck } from "react-icons/fa";
...
export default function Suppliers() {
  ...
  const fetchItems = () => {
    setLoading(true);
    listSuppliers({ text: search, pageIndex, pageSize })
  ...
  return (
    <div className="bg-white m-8 px-8 py-8 shadow-2xl rounded-lg relative">
      ...
      <div className="bg-gray-200 p-4 rounded-sm">
        ...
        {loading ? (
            ...
            {items.map((item) => (
              <div key={item.Id} className="bg-white rounded-lg shadow-lg border">
                <div className="grid grid-cols-12 gap-2 items-center py-4 px-6 hover:shadow-xl transition-all">
                  <span className="col-span-3 font-medium text-indigo-700 truncate">{item.Name}</span>
                  <span className="col-span-3 text-sm text-gray-700 truncate">{item.MobileLine || item.LandLine || item.Email || "—"}</span>
                  <span className="col-span-3 text-sm text-gray-700 truncate">{item.ChartOfAccountAccountName || "—"}</span>
                  ...
                  <span className="col-span-2 text-right flex justify-end gap-1">
                    <Button size="sm" variant="outline" onClick={() => setDrawerItem(item)} title="Edit supplier"><FaEdit /></Button>
                  </span>
                </div>
              </div>
            ))}
          ...
          <div className="text-gray-500 text-center mt-4">
            ...
            <p className="font-medium text-gray-400">No suppliers found.</p>
          </div>
        )}
      ...
      <SupplierDrawer open={drawerItem !== undefined} onClose={() => setDrawerItem(undefined)} onSuccess={fetchItems} item={drawerItem} />
    </div>
  );
}
