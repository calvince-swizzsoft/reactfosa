...
export default function EditInvJournalDrawer({ open, onClose, onSuccess, journal }) {
  ...
  const entryTypeOptions = [
    ...
    ...customEntryTypes.map((et) => ({ Id: et, Description: et })),
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_APP_INV_URL}/api/itemjournals/${journal.Id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!res.ok) throw new Error(response.message || "Failed to update journal");

      response.success && Swal.fire("Updated!", response.message, "success");
      if (onSuccess) onSuccess();
      onClose();
    } catch (err) {
      console.error(err);
      Swal.fire("Error", err.message || "Failed to update journal", "error");
    } finally {
      setLoading(false);
    }
  };
      ...
      {activeDropdown === type && (
        ...
        >
          <div className="flex justify-between items-center mb-4 bg-indigo-600 p-2 rounded-2xl text-white">
            ...
            <Button variant="ghost" size="sm" onClick={() => setActiveDropdown(null)}>
              ✕
            </Button>
          </div>
            ...
            >
              ...
              <div>
                ...
                <Button
                  ...
                  onClick={() =>
                    setActiveDropdown(activeDropdown === "entrytype" ? null : "entrytype")
                  }
                >
              ...
              <div>
                ...
                >
                  {locations.find((l) => l.Id === form.ItemLocationId)?.Description ||
                    "Select Location"}
                </Button>
              </div>
