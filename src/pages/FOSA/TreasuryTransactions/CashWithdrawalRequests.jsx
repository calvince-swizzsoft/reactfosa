...
export default function CashWithdrawalRequests() {
  ...
  const postAuthorizedRequest = async (item) => {
    const confirmed = await Swal.fire({ title: "Pay authorized withdrawal?", text: `Post and pay ${money(item.Amount)} from ${item.CustomerAccountFullAccountNumber || "the customer account"}?`, icon: "question", showCancelButton: true, confirmButtonText: "Post & Pay", confirmButtonColor: "#4f46e5" });
    if (!confirmed.isConfirmed) return;
    setPostingIds((current) => new Set(current).add(item.Id));
    try {
      const result = await apiJson(`${FIN_BASE}/api/frontoffice/requests/post?id=${encodeURIComponent(item.Id)}`, { method: "POST" });
      await Swal.fire("Withdrawal Paid", result?.message || result?.Message || "The authorized withdrawal was posted successfully.", "success");
      load();
    } catch (error) {
      Swal.fire("Unable to Pay Withdrawal", apiErrorMessage(error, "The authorized withdrawal could not be posted."), "error");
    } finally {
      setPostingIds((current) => { const next = new Set(current); next.delete(item.Id); return next; });
    }
  };
  const resendApprovalRequest = async (item) => {
    const confirmed = await Swal.fire({ title: "Resend approval request?", text: "This will create a new approval workflow only if no unactioned approval request already exists.", icon: "question", showCancelButton: true, confirmButtonText: "Resend", confirmButtonColor: "#4f46e5" });
    if (!confirmed.isConfirmed) return;
    setResendingIds((current) => new Set(current).add(item.Id));
    try {
      const result = await apiJson(`${BASE}/${encodeURIComponent(item.Id)}/resend-approval`, { method: "POST" });
      await Swal.fire("Approval Resent", result?.message || result?.Message || "The approval request was resent successfully.", "success");
      load();
    } catch (error) {
      Swal.fire("Unable to Resend Approval", apiErrorMessage(error, "The approval request could not be resent."), "error");
    } finally {
      setResendingIds((current) => { const next = new Set(current); next.delete(item.Id); return next; });
    }
  };
  return <div className="bg-white m-8 px-8 py-8 shadow-2xl rounded-lg relative">
    ...
    <div className="bg-gray-200 p-4 rounded-sm"><div className="grid grid-cols-12 gap-4 bg-gray-700 text-gray-100 font-semibold p-3 rounded-lg mb-4"><span className="col-span-2">Customer</span><span className="col-span-2">Account</span><span className="col-span-2">Amount</span><span className="col-span-2">Notice / Maturity</span><span className="col-span-1">Status</span><span className="col-span-1">Created</span><span className="col-span-2 text-right">Actions</span></div>
      {loading ? <div className="space-y-2 animate-pulse">{[1,2,3].map((key) => <div key={key} className="grid grid-cols-12 gap-3 bg-gray-50 p-6 rounded-lg">{Array.from({ length: 6 }).map((_, index) => <div key={index} className="col-span-2 h-4 bg-gray-200 rounded" />)}</div>)}</div> : items.length ? <div className="space-y-2">{items.map((item) => <div key={item.Id} onClick={() => openDecision(item)} className={`w-full text-left bg-white rounded-lg shadow-lg border transition-all ${item.Status === 1 ? "hover:shadow-xl cursor-pointer" : "cursor-default"}`}><div className="grid grid-cols-12 gap-4 items-center py-4 px-6"><span className="col-span-2 text-sm text-gray-700">{item.CustomerName || item.CustomerAccountCustomerFullName || "—"}</span><span className="col-span-2 text-sm text-gray-700">{item.CustomerAccountFullAccountNumber || "—"}</span><span className="col-span-2 text-sm font-semibold text-indigo-700">{money(item.Amount)}</span><span className="col-span-2 text-sm text-gray-700">{item.TypeDescription || "—"}<small className="block text-gray-400">{date(item.MaturityDate)}</small></span><span className="col-span-1"><span className={`px-2 py-1 rounded text-xs font-semibold ${item.Status === 2 ? "bg-green-100 text-green-600" : item.Status === 4 ? "bg-red-100 text-red-600" : item.Status === 8 ? "bg-blue-100 text-blue-600" : "bg-amber-100 text-amber-600"}`}>{item.StatusDescription || "Pending"}</span></span><span className="col-span-1 text-xs text-gray-500 break-words">{date(item.CreatedDate)}<small className="block">{item.CreatedBy || "—"}</small></span><span className="col-span-2 flex justify-end gap-1">{item.Status === 1 && <Button type="button" size="sm" variant="outline" disabled={resendingIds.has(item.Id)} onClick={(event) => { event.stopPropagation(); resendApprovalRequest(item); }} className="border-indigo-200 text-indigo-700 hover:bg-indigo-50">{resendingIds.has(item.Id) ? "Resending..." : "Resend Approval"}</Button>}{item.Status === 2 && <Button type="button" size="sm" disabled={postingIds.has(item.Id)} onClick={(event) => { event.stopPropagation(); postAuthorizedRequest(item); }} className="bg-indigo-600 hover:bg-indigo-700">{postingIds.has(item.Id) ? "Posting..." : "Post & Pay"}</Button>}</span></div></div>)}</div> : <div className="text-center text-gray-400"><img src={NotFoundImage} alt="No requests" className="mx-auto w-42" /><p>No cash withdrawal requests match the selected filters.</p></div>}
