...
export default function AddSalesInvoiceDrawer({ open, onClose, onSuccess }) {
  ...
  const handleLineChange = (index, field, value) => {
    const updatedLines = [...formData.SalesInvoiceLines];
    ...
    setFormData({
      ...formData,
      SalesInvoiceLines: updatedLines,
    });
  };
  ...
  const removeLine = (index) => {
    const updatedLines = formData.SalesInvoiceLines.filter((_, i) => i !== index);
    setFormData({ ...formData, SalesInvoiceLines: updatedLines });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await apiJson(
        `${import.meta.env.VITE_APP_FIN_URL}/api/values/AddSalesInvoice`,
        {
          method: "POST",
          headers: {
      ...
      Swal.fire("Error", apiErrorMessage(err, "Unable to add the sales invoice."), "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (showLinesDrawer && formData.SalesInvoiceLines.length > 0) {
      setExpandedIndex(0);
    }
  }, [showLinesDrawer, formData.SalesInvoiceLines.length]);

  useEffect(() => {
    // Fetch Sales Invoice Types
        ...
        <>
          ...
          >
            <div className="p-4 flex justify-between items-center bg-indigo-600 rounded-2xl m-2">
              <h2 className="font-bold text-lg text-white">Add Sales Invoice</h2>
            ...
            <div className="p-3 flex-1 overflow-y-auto">
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                  ...
                  {formData.SalesInvoiceLines.length})
                </Button>
                          ...
                          {expandedIndex === idx && (
                            ...
                            >
                              ...
                              <Select
                                ...
                                onValueChange={(value) =>
                                  handleLineChange(idx, "CreditChartOfAccountId", value)
                                }
                              >
                                <SelectTrigger>
