...
export function MemberRegistrationProvider({ children }) {
    ...
    const clearDraft = () => {
        ...
        setCustomer({ ...initialCustomerState, createdDate: new Date().toISOString().split("T")[0], registrationDate: new Date().toISOString().split("T")[0] });
    ...
    return (
        <MemberRegistrationContext.Provider
            value={{ customer, setCustomer, nextOfKins, setNextOfKins, step, setStep, hasDraft, clearDraft, initialNextOfKin }}
        >
            {children}
        </MemberRegistrationContext.Provider>
    );
