...
import { FaEdit, FaUsers, FaPlus, FaChevronLeft, FaChevronRight, FaMoneyBillAlt } from "react-icons/fa";
import { listExemptions } from "./api";
import ChargeExemptionDrawer from "./ChargeExemptionDrawer";

export default function ChargesExemptions() {
  ...
  const fetchItems = () => {
    setLoading(true);
    listExemptions({ text: search, pageIndex, pageSize })
    ...
    <div className="bg-white m-8 px-8 py-8 shadow-2xl rounded-lg relative">
      <div className="flex justify-between items-center mb-6 bg-indigo-800 px-6 py-3 rounded-2xl">
        <h2 className="text-xl font-bold text-white flex items-center gap-2">
          <FaMoneyBillAlt /> Charges Exemptions
        </h2>
      ...
      <div className="flex flex-wrap justify-between items-center mb-4 gap-3">
        <Input
          ...
          placeholder="Search by description or commission..."
          className="max-w-xs"
        />
      </div>
      ...
      <div className="bg-gray-200 p-4 rounded-sm">
        <div className="grid grid-cols-12 gap-4 bg-gray-700 text-gray-100 font-semibold p-3 rounded-lg mb-4">
          ...
          <span className="col-span-4">Commission</span>
        ...
        {loading ? (
            ...
            {items.map((item) => (
              <div key={item.Id} className="bg-white rounded-lg shadow-lg border">
                <div className="grid grid-cols-12 gap-2 items-center py-4 px-6 hover:shadow-xl transition-all">
                  ...
                  <span className="col-span-4 text-sm text-gray-700 truncate">{item.CommissionDescription || "—"}</span>
                  ...
                  <span className="col-span-3 text-right flex justify-end gap-1">
                    <Button size="sm" variant="outline" onClick={() => setDrawerItem(item)} title="View exempted customers"><FaUsers /></Button>
                    <Button size="sm" variant="outline" onClick={() => setDrawerItem(item)} title="Edit exemption"><FaEdit /></Button>
                  </span>
                </div>
              </div>
            ))}
          ...
          <div className="text-gray-500 text-center mt-4">
            ...
            <p className="font-medium text-gray-400">No charge exemptions found.</p>
          </div>
        )}
      ...
      <ChargeExemptionDrawer open={drawerItem !== undefined} onClose={() => setDrawerItem(undefined)} onSuccess={fetchItems} item={drawerItem} />
    </div>
  );
}
