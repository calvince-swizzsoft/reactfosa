...
import { EMPLOYERS_BASE, listDivisionsForEmployer } from "./api";

// Two-level lookup per Areas/Registry/Directors.md: "select the name of the
// Employer first, then on the 'Divisions' pane select the division that the
// Director belongs to." Same shape as Delegates' ZoneLookupModal — step 1
// lists employers (GET employer/all), step 2 lists that employer's
// divisions (GET employer/{id}/divisions, already built for
// Employer/index.jsx's own drilldown).
export default function DivisionLookupModal({ onSelect, onClose }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setQuery("");
    listDivisionsForEmployer(employer.Id)
      .then((rows) => { if (!cancelled) setDivisions(rows); })
  ...
  const list = employer ? divisions : employers;
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-[480px] max-h-[80vh] flex flex-col z-10">
        <div className="flex justify-between items-center px-5 py-4 bg-indigo-600 rounded-t-2xl">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-white text-base">{employer ? `Divisions — ${employer.Description}` : "Select Employer"}</h3>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2">
          {loading ? (
            ...
            <p className="text-center text-sm text-gray-400 py-8">{employer ? "No divisions found for this employer." : "No employers found."}</p>
          ) : (
            filtered.map((item) => (
              <button
                key={item.Id}
