...
import { FaEdit, FaPlus, FaChevronLeft, FaChevronRight, FaUsers } from "react-icons/fa";
import { listDelegates } from "./api";
...
export default function Delegates() {
  ...
  const fetchItems = () => {
    setLoading(true);
    listDelegates({ text: search, pageIndex, pageSize })
    ...
    <div className="bg-white m-8 px-8 py-8 shadow-2xl rounded-lg relative">
      ...
      <div className="bg-gray-200 p-4 rounded-sm">
        <div className="grid grid-cols-12 gap-4 bg-gray-700 text-gray-100 font-semibold p-3 rounded-lg mb-4">
          ...
          <span className="col-span-4">Zone</span>
        ...
        {loading ? (
            ...
            {items.map((item) => (
              <div key={item.Id} className="bg-white rounded-lg shadow-lg border">
                <div className="grid grid-cols-12 gap-2 items-center py-4 px-6 hover:shadow-xl transition-all">
                  <span className="col-span-4 font-medium text-indigo-700 truncate">{item.CustomerFullName?.trim() || item.FullName?.trim() || "—"}</span>
                  <span className="col-span-4 text-sm text-gray-700 truncate">{item.ZoneDescription || "—"}</span>
                  ...
                  <span className="col-span-3 text-right flex justify-end gap-1">
                    <Button size="sm" variant="outline" onClick={() => setDrawerItem(item)} title="Edit delegate"><FaEdit /></Button>
                  </span>
                </div>
              </div>
            ))}
          ...
          <div className="text-gray-500 text-center mt-4">
            ...
            <p className="font-medium text-gray-400">No delegates found.</p>
          </div>
        )}
      ...
      <DelegateDrawer open={drawerItem !== undefined} onClose={() => setDrawerItem(undefined)} onSuccess={fetchItems} item={drawerItem} />
    </div>
  );
}
