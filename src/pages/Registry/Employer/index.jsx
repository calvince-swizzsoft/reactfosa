...
import { FaBuilding, FaPlus, FaEllipsisV, FaEdit, FaTrash, FaChevronLeft, FaChevronRight } from "react-icons/fa";
import {
...
export default function Employers() {
  ...
  const fetchItems = () => {
    ...
    apiFetch(`${EMPLOYER_BASE}?${params.toString()}`)
      .then((r) => r.json())
      .then((d) => {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      text: "This also removes its divisions, zones, and stations.",
      icon: "warning",
      ...
      if (r.isConfirmed) {
        try {
          const res = await apiFetch(`${EMPLOYER_BASE}/${id}`, { method: "DELETE" });
            ...
            {items.map((item) => (
              <div key={item.Id} className="bg-white rounded-lg shadow-lg border">
                <div className="grid grid-cols-12 gap-2 items-center py-4 px-6 hover:shadow-xl transition-all">
                  ...
                  <span className="col-span-3 text-sm text-gray-600">{item.AddressCity || "—"}</span>
