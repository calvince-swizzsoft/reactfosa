...
import { FaMapMarkedAlt, FaPlus, FaEllipsisV, FaEdit, FaTrash, FaChevronLeft, FaChevronRight } from "react-icons/fa";
import {
...
export default function Zones() {
  ...
  const fetchItems = () => {
    ...
    apiFetch(`${ZONE_BASE}?${params.toString()}`)
      .then((r) => r.json())
      .then((d) => {
  ...
  const handleDelete = (id) => {
    Swal.fire({ title: "Delete Zone?", icon: "warning", showCancelButton: true, confirmButtonColor: "#dc2626", confirmButtonText: "Delete" }).then(async (r) => {
      if (r.isConfirmed) {
        try {
          const res = await apiFetch(`${ZONE_BASE}/${id}`, { method: "DELETE" });
          if (!res.ok) throw new Error("Failed to delete");
          setItems((prev) => prev.filter((x) => x.Id !== id));
          Swal.fire("Deleted!", "Zone removed.", "success");
        } catch (err) {
            ...
            {items.map((item) => (
              <div key={item.Id} className="bg-white rounded-lg shadow-lg border">
                <div className="grid grid-cols-12 gap-2 items-center py-4 px-6 hover:shadow-xl transition-all">
                  ...
                  <span className="col-span-4 text-sm text-gray-600">{item.DivisionDescription || "—"}</span>
