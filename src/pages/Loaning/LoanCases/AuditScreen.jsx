        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {loading ? (
            ...
            <>
              ...
              {activeTab === "history" && <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><section><Heading>Standing Orders</Heading><Rows items={data.standingOrders} empty="No standing orders found." render={(item) => <StandingOrderSummary item={item} accounts={data.accounts || data.loanAccounts} />} /></section><section><Heading>Posted Payouts</Heading><Rows items={data.payouts} empty="No posted payouts found." render={(item) => <PayoutSummary item={item} accounts={data.accounts || data.loanAccounts} />} /></section><section className="md:col-span-2"><Heading>Applications in process</Heading><Rows items={data.applications} empty="No other applications in process." render={(item) => `${item.PaddedCaseNumber || "Loan case"} · ${item.LoanProductDescription || ""} · ${item.StatusDescription || ""}`} /></section></div>}
