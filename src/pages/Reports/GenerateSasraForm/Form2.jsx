...
export default function Form2({ onClose, savedVersionId, savedForm6VersionId }) {
  ...
  useEffect(() => {
    ...
    Promise.all([request(savedVersionId ? `/versions/${savedVersionId}` : "/form2/definition"), savedVersionId ? (savedForm6VersionId ? request(`/versions/${savedForm6VersionId}`) : Promise.resolve({ revision: 0 })) : request("/form6/definition")]).then(([d, sofp]) => { if (active) { setDefinition(d); setSourceForm6(sofp); } }).catch(e => { if (active) setError(e.message); }).finally(() => { if (active) setBusy(false); });
    return () => { active = false; };
  ...
  function download() {
    ...
    const link = document.createElement("a"); link.href = url; link.download = `SASRA-Form2-${result.asAt.slice(0, 10)}.xls`; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000);
  }
  return <main className="bg-white m-8 px-8 py-8 shadow-2xl rounded-lg relative">
    <header className="bg-indigo-800 px-6 py-3 rounded-2xl flex justify-between items-center gap-3"><h1 className="text-xl font-bold text-white flex items-center gap-2"><FaFileExcel />Form 2 · Liquidity Statement</h1><Button variant="outline" disabled={busy} onClick={close}>Back to SASRA reports</Button></header>
    <div className="flex flex-wrap justify-between gap-3 my-4 text-sm text-gray-600"><FieldLabel label="DT SACCO · KSh · All branches" help="Form 2 uses whole KSh and closing balances across all branches. Form 6 supplies the mapping comparison. This report is generated from saved mappings without additional maturity or eligibility adjustments. Complete those in the downloaded Excel file. Excel changes are not saved back into the system." /><span>{definition?.id && definition.id !== "00000000-0000-0000-0000-000000000000" ? `Saved revision ${definition.revision}` : "Mappings not saved"}{dirty ? " · Unsaved changes" : ""}</span></div>
    <div className="my-3 text-sm text-gray-700"><FieldLabel label="Generated from mappings · Adjustments completed in Excel" help="In worksheet Liquidity, review D15 (long-term bank deposits), D16 (overdrafts and matured bank obligations), D24 (matured non-bank borrowings), D37–D39 (deposit deductions) and D44–D45 (other liability maturities). Adjust mapped input balances for uncleared or restricted amounts where applicable. Schedule amounts initially show zero because no additional adjustments were supplied; D16 already includes mapped bank overdrafts. Preserve total and ratio formulas and retain your supporting schedules." /></div>
    ...
    {!definition ? busy && <div className="animate-pulse h-32 bg-gray-100 rounded-lg" /> : <>
      <form className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end border rounded-lg p-4 mb-4" onSubmit={e => {
        e.preventDefault();
        if (dirty || !definition.revision || !sourceForm6?.revision) { setError("Save Form 2 mappings and a Form 6 revision before previewing."); return; }
        const dateError = validateForm6Dates(yearStart, asAt);
        if (dateError) { setError(dateError); return; }
        run(async () => { setResult(null); const data = await request("/form2/preview", { method: "POST", body: JSON.stringify({ versionId: definition.id, form6VersionId: sourceForm6.id, yearStart, asAt, mappingBased: true }) }); setResult(data); chooseView("preview"); });
      }}>
        ...
        <Button className={primary} disabled={busy || dirty || !definition.revision || !sourceForm6?.revision} type="submit">{busy ? "Working…" : "Preview Form 2"}</Button>
        <Button type="button" variant="outline" disabled={busy || dirty || !result?.workbookBase64} onClick={download}>Download Excel</Button>
        <p className="col-span-full text-sm text-gray-500">{sourceForm6?.revision ? `Balance-sheet comparison: Form 6 revision ${sourceForm6.revision}.` : "Save a Form 6 workbook revision before previewing Form 2."}</p>
      </form>
      {result && <div className="border rounded-lg p-3 mb-4 text-sm space-y-2">
        ...
        <div className="flex flex-wrap gap-6"><FieldLabel label={`Deposit reconciliation difference: ${money(result.difference)}`} help="Gross deposits before exclusions minus deposits from the selected Form 6 mappings, in KSh. Differences above KSh 0.01 block export. The official ratio uses gross deposits plus other short-term liabilities; a correctly calculated deficit remains reportable." /><span>Ledger difference: {money(result.ledgerDifference)}</span><FieldLabel label={`Automatic bank overdrafts: ${money(result.bankOverdrafts)}`} help="Credit balances from accounts mapped to commercial banks, deducted once from liquid assets. In Excel, D16 already includes this amount. Add only additional bank obligations so the overdraft is not counted twice." /></div>
      ...
      <div className="flex flex-wrap justify-between items-center gap-3 mb-3"><div className="flex gap-2"><Button variant={view === "mappings" ? "default" : "outline"} onClick={() => chooseView("mappings")}>Account mappings</Button>{result && <><Button variant={view === "preview" ? "default" : "outline"} onClick={() => chooseView("preview")}>Preview</Button><Button variant={view === "unmapped" ? "default" : "outline"} onClick={() => chooseView("unmapped")}>Unmapped ({result.unmappedAccounts.length})</Button></>}</div>{view === "mappings" && <Button className={primary} disabled={busy || (!dirty && definition.revision > 0)} onClick={() => run(async () => { const saved = await request("/versions", { method: "POST", body: JSON.stringify(definition) }); setDefinition(saved); setDirty(false); setResult(null); setNotice("Form 2 mappings saved as a new revision."); })}>Save mappings</Button>}</div>
      {view === "mappings" && <div className="mb-3"><FieldLabel label="Map posting accounts to each report line" help="Map cash, bank placements, government securities and deposits to the appropriate lines. Add accrued-interest accounts to their related balance where applicable. Complete exclusions and maturity adjustments in Excel after download. Non-liquid assets such as member loans and fixed assets do not belong here. Other liability maturities need supporting schedules. Foreign currency cash must already be translated to KSh." /></div>}
