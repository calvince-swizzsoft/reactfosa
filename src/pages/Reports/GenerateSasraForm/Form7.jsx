...
import { validateForm6Dates as validateReportDates } from "./form6Model";
...
export default function Form7({ onClose, savedVersionId }) {
  ...
  useEffect(() => {
    ...
    request(savedVersionId ? `/versions/${savedVersionId}` : "/form7/definition").then(d => { if (active) setDefinition(d); }).catch(e => { if (active) setError(e.message); }).finally(() => { if (active) setBusy(false); });
    return () => { active = false; };
  ...
  const mappingLines = definition?.lines.filter(l => l.source === "GlMovement") || [];
  ...
  function download() {
    ...
    const link = document.createElement("a"); link.href = url; link.download = `SASRA-Form7-${result.asAt.slice(0, 10)}.xls`; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000);
  }
  return <main className="bg-white m-8 px-8 py-8 shadow-2xl rounded-lg relative">
    <header className="bg-indigo-800 px-6 py-3 rounded-2xl flex justify-between items-center gap-3"><h1 className="text-xl font-bold text-white flex items-center gap-2"><FaFileExcel />Form 7 · Statement of Comprehensive Income</h1><Button variant="outline" disabled={busy} onClick={close}>Back to SASRA reports</Button></header>
    <div className="flex flex-wrap justify-between gap-3 my-4 text-sm text-gray-600"><FieldLabel label="DT SACCO · KSh thousands · All branches" help="The official Form 7 workbook retains its rows and formulas. Income and expense activity runs from the financial-year start through the whole as-at day. Fiscal-period closing transfers are excluded from activity and shown separately in reconciliation. Download uses the same calculated values as this preview." /><span>{definition?.id && definition.id !== "00000000-0000-0000-0000-000000000000" ? `Saved revision ${definition.revision}` : "Mappings not saved"}{dirty ? " · Unsaved changes" : ""}</span></div>
    ...
    {!definition ? busy && <div className="animate-pulse h-32 bg-gray-100 rounded-lg" /> : <>
      <form className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end border rounded-lg p-4 mb-4" onSubmit={e => {
        ...
        const dateError = validateReportDates(yearStart, asAt);
        if (dateError) { setError(dateError); return; }
        run(async () => { setResult(null); const data = await request("/form7/preview", { method: "POST", body: JSON.stringify({ versionId: definition.id, yearStart, asAt }) }); setResult(data); chooseView("preview"); });
      }}>
        <div><FieldLabel label="Financial year start" help="Enter the actual start of the SACCO's financial year. Form 7 reports income and expenses since this date, excluding fiscal-period closing transfers. Use the same dates as Form 6 to compare the surplus figures." /><Input aria-label="Financial year start" type="date" required disabled={busy} value={yearStart} onChange={e => { setYearStart(e.target.value); setResult(null); chooseView("mappings"); }} /></div>
        <div><FieldLabel label="As at" help="Income and expense activity includes the whole selected day, using journal value date with created date as fallback. Earlier financial years are excluded." /><Input aria-label="As at" type="date" required min={yearStart || undefined} disabled={busy} value={asAt} onChange={e => { setAsAt(e.target.value); setResult(null); chooseView("mappings"); }} /></div>
        <Button className={primary} disabled={busy || dirty || !definition.revision} type="submit">{busy ? "Working…" : "Preview Form 7"}</Button>
      ...
      {result && <div className="border rounded-lg p-3 mb-4 text-sm space-y-2">
        ...
        <div className="flex flex-wrap gap-6"><FieldLabel label={`Reconciliation difference: ${money(result.difference)}`} help="Form 7 net income minus the income and expense ledger result before closing transfers, in KSh thousands. Download requires a difference within KSh 0.01, a balanced period ledger and all relevant income and expense accounts mapped." /><span>Ledger difference: {money(result.ledgerDifference)}</span><span>Ledger net income: {money(result.ledgerNetIncome)}</span><FieldLabel label={`Unclosed surplus: ${money(result.unclosedSurplus)}`} help="This uses the same year-to-date income and expense basis as Form 6 current-year surplus, including posted closing transfers. It is calculated independently of Form 6 mappings." /><FieldLabel label={`Closing adjustment: ${money(result.closingAdjustment)}`} help="Add this adjustment to unclosed surplus to obtain Form 7 ledger net income. System fiscal-period closing journals and their reversals retain transaction code 15 and are excluded from Form 7 activity. They remain in the ledger and Form 6." /></div>
        {result.issues.map(issue => <p key={issue} role="alert" className="text-red-700">{issue}</p>)}
      </div>}
      <div className="flex flex-wrap justify-between items-center gap-3 mb-3"><div className="flex gap-2"><Button variant={view === "mappings" ? "default" : "outline"} onClick={() => chooseView("mappings")}>Account mappings</Button>{result && <><Button variant={view === "preview" ? "default" : "outline"} onClick={() => chooseView("preview")}>Preview</Button><Button variant={view === "unmapped" ? "default" : "outline"} onClick={() => chooseView("unmapped")}>Unmapped ({result.unmappedAccounts.length})</Button></>}</div>{view === "mappings" && <Button className={primary} disabled={busy || (!dirty && definition.revision > 0)} onClick={() => run(async () => { const saved = await request("/versions", { method: "POST", body: JSON.stringify(definition) }); setDefinition(saved); setDirty(false); setResult(null); setNotice("Form 7 mappings saved as a new revision."); })}>Save mappings</Button>}</div>
      {view === "mappings" && <div className="mb-3"><FieldLabel label="Map posting accounts to each report line" help="Map each income or expense posting account once. Income uses credit-positive presentation and expenses debit-positive. Loan-loss recoveries reduce the provision expense; donations are net income less donation expense. Use income or contra-expense recovery accounts, not loan principal. Leave genuinely unused lines without accounts. The preview identifies unmapped accounts with non-zero net activity. Workbook totals need no mappings." /></div>}
      <div className="bg-gray-200 p-4 rounded-sm"><div className="grid grid-cols-12 gap-3 bg-gray-700 text-gray-100 font-semibold p-3 rounded-lg mb-3"><span className="col-span-5">{view === "unmapped" ? "Account" : "Report line"}</span><span className={view === "mappings" ? "col-span-6" : "col-span-7 text-right"}>{view === "mappings" ? "Mapped accounts" : "KSh thousands"}</span></div>
        ...
        {!rows.length && <div className="text-center py-6"><img src="/assets/scopefinding.png" alt="" className="mx-auto w-32" /><p className="text-gray-400">{view === "unmapped" ? "No unmapped income or expense balances." : "No lines to display."}</p></div>}
      </div>
