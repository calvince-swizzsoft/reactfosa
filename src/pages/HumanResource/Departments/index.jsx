...
import { FaEdit, FaPlus, FaBuilding } from "react-icons/fa";
import { apiFetch, normalizeList } from "@/lib/api";

// Areas/HumanResource/Controllers/DepartmentsController.cs — GET/POST/PUT
// only, no DELETE route and no paging/text-filter support (Index() returns
// the plain full FindDepartments() list) — no Prev/Next or server search
...
const DEPARTMENTS_BASE = `${BASE}/api/humanresource/departments`;
...
function EditDepartmentDrawer({ open, onClose, onSuccess, item }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await apiFetch(`${DEPARTMENTS_BASE}/${item.Id}`, {
        method: "PUT",
      ...
      <div className="bg-gray-200 p-4 rounded-sm">
        ...
        {loading ? (
          ...
          <div className="text-gray-500 text-center mt-4">
            ...
            <p className="font-medium text-gray-400">No departments found.</p>
          </div>
        )}
      </div>
