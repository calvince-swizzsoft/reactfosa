...
function EditLeaveApplicationDrawer({ open, onClose, onSuccess, item }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await updateLeaveApplication(item.Id, {
        EmployeeId: item.EmployeeId,
        LeaveTypeId: form.LeaveTypeId,
        DurationStartDate: form.DurationStartDate,
          ...
          <motion.div className="fixed top-5 right-3 w-[420px] bg-white shadow-xl z-50 flex flex-col rounded-2xl p-3" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 300, damping: 30 }}>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              ...
              <FieldGroup label="Leave Type">
                <Select value={form.LeaveTypeId} onValueChange={(v) => setForm((p) => ({ ...p, LeaveTypeId: v }))} disabled={loadingData}>
                  ...
                  <SelectContent className="max-h-60 overflow-y-auto">
                    {leaveTypes.map((lt) => (
                      <SelectItem key={lt.Id} value={lt.Id}>{lt.Description}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </FieldGroup>
              ...
              <FieldGroup label="Start Date">
                <Input type="date" value={form.DurationStartDate} onChange={(e) => setForm((p) => ({ ...p, DurationStartDate: e.target.value }))} required />
              </FieldGroup>

              <FieldGroup label="End Date">
                <Input type="date" value={form.DurationEndDate} min={form.DurationStartDate} onChange={(e) => setForm((p) => ({ ...p, DurationEndDate: e.target.value }))} required />
              </FieldGroup>
...
export default function LeaveApplicationList() {
  ...
  const fetchItems = () => {
    setLoading(true);
    listLeaveApplications({ text: search, pageIndex, pageSize })
      .then((page) => {
      ...
      <div className="bg-gray-200 p-4 rounded-sm">
        ...
        {loading ? (
              ...
              <div key={item.Id} className="bg-white rounded-lg shadow-lg border">
                <div className="grid grid-cols-12 gap-2 items-center py-4 px-6 hover:shadow-xl transition-all">
                  ...
                  <span className="col-span-2">
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${LEAVE_STATUS_BADGE_CLASS[item.Status] || "bg-gray-100 text-gray-500"}`}>
          ...
          <div className="text-gray-500 text-center mt-4">
            ...
            <p className="font-medium text-gray-400">No leave applications found.</p>
          </div>
        )}
