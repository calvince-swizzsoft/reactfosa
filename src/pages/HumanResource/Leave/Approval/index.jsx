...
async function promptDecision(item, decision) {
  ...
  const { value: remarks } = await Swal.fire({
    title: isApprove ? `Approve leave for ${item.EmployeeCustomerFullName?.trim()}?` : `Reject leave for ${item.EmployeeCustomerFullName?.trim()}?`,
    input: "textarea",
...
export default function LeaveApprovalList() {
  ...
  const handleDecision = async (item, decision) => {
    const remarks = await promptDecision(item, decision);
            ...
            {items.map((item) => (
              <div key={item.Id} className="bg-white rounded-lg shadow-lg border">
                <div className="grid grid-cols-12 gap-2 items-center py-4 px-6 hover:shadow-xl transition-all">
                  ...
                  <span className="col-span-2 text-sm text-gray-700">{formatDate(item.DurationStartDate)} – {formatDate(item.DurationEndDate)}</span>
                  <span className="col-span-2 text-sm text-gray-500 truncate">{item.Reason || "—"}</span>
