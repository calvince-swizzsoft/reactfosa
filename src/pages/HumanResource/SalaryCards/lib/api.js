...
export function listSalaryCards({ text = "", pageIndex = 0, pageSize = 20 } = {}) {
  ...
  return unwrapJson(apiJson(`${SALARY_CARDS_BASE}?${params.toString()}`));
}

export function getSalaryCard(id) {
  return unwrapJson(apiJson(`${SALARY_CARDS_BASE}/${id}`));
}
...
export async function getSalaryCardByEmployee(employeeId) {
  try {
    return await apiJson(`${SALARY_CARDS_BASE}/by-employee/${employeeId}`);
  } catch (error) {
...
export function createSalaryCard(payload) {
  return unwrapJson(apiJson(SALARY_CARDS_BASE, { method: "POST", body: JSON.stringify(payload) }));
}

export function updateSalaryCard(id, payload) {
  return unwrapJson(apiJson(`${SALARY_CARDS_BASE}/${id}`, { method: "PUT", body: JSON.stringify({ ...payload, Id: id }) }));
}

export function listCardEntries(cardId) {
  return unwrapJson(apiJson(`${SALARY_CARDS_BASE}/${cardId}/entries`));
}

export function resetCardEntries(cardId) {
  return unwrapJson(apiJson(`${SALARY_CARDS_BASE}/${cardId}/reset-entries`, { method: "POST" }));
}
...
export function updateCardEntry(entryId, payload) {
  return unwrapJson(apiJson(`${SALARY_CARDS_BASE}/entries/${entryId}`, { method: "PUT", body: JSON.stringify({ ...payload, Id: entryId }) }));
}
