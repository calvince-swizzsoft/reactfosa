...
export function updateChequeBook(id, chequeBookDTO) {
  return unwrap(apiJson(`${CHEQUEBOOKS_BASE}/${id}`, { method: "PUT", body: JSON.stringify(chequeBookDTO) }));
}

export function listVouchers(chequeBookId, { text = "", pageIndex = 0, pageSize = 20 } = {}) {
  const params = new URLSearchParams({ text, pageIndex: String(pageIndex), pageSize: String(pageSize) });
  return unwrap(apiJson(`${CHEQUEBOOKS_BASE}/${chequeBookId}/vouchers?${params.toString()}`));
}
...
export function matchVoucher({ chequeBookType, voucherNumber, chequeBookReference }) {
  ...
  return unwrap(apiJson(`${CHEQUEBOOKS_BASE}/vouchers/match?${params.toString()}`));
}
...
export function payVoucher(id, paymentVoucherDTO) {
  return unwrap(apiJson(`${CHEQUEBOOKS_BASE}/vouchers/${id}/pay`, { method: "POST", body: JSON.stringify(paymentVoucherDTO) }));
}
...
export function flagVoucher(id, paymentVoucherDTO) {
  return unwrap(apiJson(`${CHEQUEBOOKS_BASE}/vouchers/${id}/flag`, { method: "POST", body: JSON.stringify(paymentVoucherDTO) }));
}
