...
export const listEntries = (id, { text = "", pageIndex = 0, pageSize = 20 } = {}) => {
  const query = new URLSearchParams({ text, pageIndex: String(pageIndex), pageSize: String(pageSize) });
...
export const getPeriod = (id) => unwrap(apiJson(`${BASE}/periods/${id}`));
export const getBankBalance = (bankLinkageId, endDate) => unwrap(apiJson(`${BASE}/balance?${new URLSearchParams({ bankLinkageId, endDate })}`));
