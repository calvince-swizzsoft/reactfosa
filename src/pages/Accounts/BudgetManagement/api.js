...
export const listBudgets = ({ text = "", pageIndex = 0, pageSize = 100 } = {}) => {
  const query = new URLSearchParams({ pageIndex, pageSize });
  if (text?.trim()) query.set("text", text.trim());
  return unwrap(apiJson(`${BASE}?${query}`));
};
export const listAllBudgets = () => unwrap(apiJson(`${BASE}/all`));
export const getBudgetActuals = (id, asAt) => unwrap(apiJson(`${BASE}/${id}/actuals?${new URLSearchParams({ asAt })}`));
